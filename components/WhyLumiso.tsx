...
const WhyLumiso: React.FC = () => {
  ...
  const beforePoints = t('whyLumiso.before.points') as unknown as { title: string, description?: string }[];
  const afterPoints = t('whyLumiso.after.points') as unknown as { title: string, description?: string }[];
  ...
  return (
    <section id="why-lumiso" className="py-20 sm:py-32 bg-slate-100 dark:bg-slate-900 overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={headerRef} className={`max-w-3xl mx-auto ${animationClasses(isHeaderVisible)}`}>
          <SectionHeader
            align="center"
            badgeText={t('whyLumiso.tag')}
            title={t('whyLumiso.title')}
            subtitle={t('whyLumiso.subtitle')}
          />
        </div>
        ...
        <div className="mt-16 grid grid-cols-1 lg:grid-cols-[1fr,auto,1fr] gap-8 lg:gap-12 items-center">
          ...
          <div ref={beforeCardRef} className={`bg-white dark:bg-slate-800/50 p-8 rounded-2xl shadow-xl border border-slate-200/80 dark:border-slate-700/50 h-full ${animationClasses(isBeforeCardVisible, 'delay-100')}`}>
            <h3 className="text-2xl font-bold text-center text-slate-800 dark:text-slate-100">{t('whyLumiso.before.title')}</h3>
            <ul className="mt-8 space-y-6">
              {beforePoints.map((point, index) => (
                <li key={index} className="flex">
                  <XCircleIcon className="w-6 h-6 text-rose-500/80 dark:text-rose-400/80 flex-shrink-0 mt-1" />
                  <div className="ml-4">
                    <h4 className="text-slate-700 dark:text-slate-200">{point.title}</h4>
                    {point.description && (
          ...
          <div ref={afterCardRef} className={`bg-white dark:bg-slate-900/50 p-8 rounded-2xl shadow-2xl border-2 border-brand-teal-500/50 dark:border-brand-teal-400/50 h-full ${animationClasses(isAfterCardVisible, 'delay-300')}`}>
            <h3 className="text-2xl font-bold text-center text-slate-900 dark:text-white">{t('whyLumiso.after.title')}</h3>
            <ul className="mt-8 space-y-6">
        ...
        <div className="mt-16 text-center">
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-x-6 gap-y-3 text-base font-medium text-slate-600 dark:text-slate-300">
            <div className="flex items-center gap-2">
              <CheckCircleIcon className="w-5 h-5 text-brand-teal-500 flex-shrink-0" />
